...
def create_bande(image, centroids):
    # Create an image of horizontal color bands
    num_bands = len(centroids)
    palette_bands = np.zeros((num_bands * 50, 100, 3), dtype=np.uint8)
    for i, centroid in enumerate(centroids):
        palette_bands[:, i*50:(i+1)*50] = np.uint8(centroid * 255)
    ...
    cv2.imshow("Image paetete", palette_bands)
    cv2.waitKey(0)
...
def kmeans_segmentation(image, K):
    ...
    if np.isnan(centroids).any():
        ...
        kmeans_segment_and_display("./3.jpg", K)
        return
    else:
...
def kmeans_segment_and_display(image_path, K):
    ...
    palette_bands = create_bande(image, centroids)
...
kmeans_segment_and_display("./3.jpg", 6)
